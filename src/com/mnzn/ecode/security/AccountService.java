...
public class AccountService {
	...
	public User getUser(Long localid) {
		return userDao.findOne(localid);
	}

	
	public User findMobileUser(String mobile){
		return userDao.findMobileUser(mobile);
	}
	...
	public void deleteUser(Long localid) {
		/*if (isSupervisor(localid)) {
			logger.warn("操作员{}尝试删除超级管理员用户", getCurrentUserName());
			throw new ServiceException("不能删除超级管理员用户");
		}
		userDao.delete(localid);
		taskDao.deleteByUserId(localid);*/
	 ...
	 * 判断是否超级管理员.
	 */
	private boolean isSupervisor(Long localid) {
		return localid == 1;
	}
